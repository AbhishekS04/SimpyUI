		...
		<div className="relative">
			...
			>
				{files.slice(0, 6).map((file, i) => (
					<motion.div
						key={i}
						className="text-gray-100 font-mono text-xs py-0.5 whitespace-nowrap flex items-center gap-2"
						initial={{ opacity: 0 }}
						animate={{ opacity: isHovered ? 1 : 0 }}
						transition={{ duration: 0.05, delay: i * 0.03 }}
					>
						{file.icon && <span className="w-3 h-3 flex items-center">{file.icon}</span>}
						{file.name}
					</motion.div>
				))}
			</motion.div>
			...
			>
				{files.slice(0, 6).map((file, i) => (
					<div key={i} className="text-center">
						{file.icon ? (
							<div className="w-5 h-5 mx-auto flex items-center justify-center text-gray-400">
								{file.icon}
							</div>
						) : (
							<FileIcon
	...
	(
		{
			...
			files = [],
			name,
			label, // Allow direct label prop or fallback to name
			size = "md",
			className,
			onClick,
		},
		...
		const displayLabel = label || name; // Support both

		// Auto-select variant if not provided, based on input data
		const activeVariant = variant || (files.length > 0 ? "nandi" : "devi");

		const colors = variantColors[activeVariant];
		...
		const renderFolder = () => {
			const props = { isHovered, colors, sizes, label: displayLabel };
